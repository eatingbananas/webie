...
(function _mirrorLoop() {
  ...
  for (const m of _btnMirrors) {
    ...
    m.el.style.transform       = z !== 1 ? 'scale(' + z + ')' : '';
    m.el.style.transformOrigin = 'top left';
...
let _mobSpacerW = 0;  // mobile: narrowed spacer width set in finish(), preserved across zoom
...
function waitResolveAndCache() {
  ...
  function finish() {
    ...
    if (IS_MOBILE) {
      ...
      // Also store narrowW so updateSpacer() can use it on subsequent zoom calls.
      _mobSpacerW = narrowW;
...
function updateSpacer() {
  const baseW = (IS_MOBILE && _mobSpacerW > 0) ? _mobSpacerW : surfW * _currentScale;
  spacer.style.width  = Math.max(baseW, scrollWrap.clientWidth)  + 'px';
...
function applyScale(newScale, stageX, stageY) {
  ...
  const newScrollLeft = Math.max(0, stageX * (newScale - _currentScale) + scrollWrap.scrollLeft);
  const newScrollTop  = Math.max(0, stageY * (newScale - _currentScale) + scrollWrap.scrollTop);
